...
function GalleryItem({ href, title, src }) {
  return (
    <figure className="h-full space-y-4 2xl:space-y-5 3xl:space-y-6 animate-popUp">
      <Link href={href}>
        <span className="relative inline-block h-[90%] group">
          <img
            src={src}
            alt={`${title} project`}
          ...
          <span className="absolute inset-0 flex items-center justify-center w-full h-full">
            <span className="flex items-center justify-center transition duration-300 scale-0 rounded-full md:group-hover:scale-100 group-hover:scale-75 size-24 bg-green">
